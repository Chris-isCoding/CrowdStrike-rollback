...
if (-not (Test-SafeMode)) {
    Write-Host "The system is not in Safe Mode. Please restart in Safe Mode or Windows Recovery Environment." -ForegroundColor Red
    exit
}
...
if (Test-Path -Path $directoryPath) {
    # Search for and delete files matching the pattern
